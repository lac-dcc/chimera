...
def copy_verilog_files(repo, repo_path, target_dir, ext, curr_num_programs):
    for i, file in enumerate(
        (
            file
            for result in os.walk(repo_path)
        ...
        split.insert(0, str(curr_num_programs + i))
        ...
        if len(filename) > os.pathconf("/", "PC_NAME_MAX"):
            ...
            filename = "_".join([dir_name[:3] for dir_name in split[2:-1]])
            filename = os.path.join(
                target_dir, f"{split[0]}_{split[1]}_{filename}_{split[-1]}"
            )
...
def get_num_programs(extensions, target_dir):
    return reduce(
        lambda prev, ext: prev + len(glob.glob(os.path.join(target_dir, ext))),
        extensions,
        0,
    )
...
def load_current_repos(cloned_repos_savefile):
    if os.path.exists(cloned_repos_savefile):
        with open(cloned_repos_savefile, "r") as savefile:
            current_repos = set([line.strip() for line in savefile.readlines()])
    else:
        current_repos = set()
    ...
    result = run(
        "git remote get-url origin", shell=True, capture_output=True, encoding="utf-8"
    )
    regex = re.search(r"git@github\.com:(.+?\/.+?)\.git", result.stdout.strip())
    if regex:
...
def mine_from_repos(
    ...
    for repo in (repo for repo in repos_search if can_clone_repo(repo, current_repos)):
        ...
        for ext in extensions:
            ...
            copy_verilog_files(repo, repo_path, target_dir, ext, num_programs)
        print("done\n")
...
def main():
    ...
    num_programs = mine_from_repos(
        repos_search,
        current_repos,
        cloned_repos_savefile,
        extension_dict[args.language],
        args.max_programs,
        args.clones_dir,
        args.target_dir,
        args.language,
