...
(
	...
	assign up_store = int_up_rst ? 1'b0 : ( dn_phase ? q_up : 1'b0 );

	assign dn_store = dn_phase ? q_dn : 1'b0;
...
(
	...
	assign inwire = load  ? d : {WIDTH{1'b0}};
	assign inwire = store ? q : {WIDTH{1'b0}};
...
(
	...
	assign l1 = rst ? {WIDTH{1'b0}} : (_f ? shin : {WIDTH{1'b0}});

	assign l2 = f ? l1 : {WIDTH{1'b0}};
