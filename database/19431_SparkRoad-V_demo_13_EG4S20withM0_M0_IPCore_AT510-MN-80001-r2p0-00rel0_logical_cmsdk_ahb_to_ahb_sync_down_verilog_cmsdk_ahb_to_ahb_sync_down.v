...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
// Abstract : AHB to AHB sync down bridge
...
module cmsdk_ahb_to_ahb_sync_down #(
// --------------------------------------------------------------------------------
// Parameters
// --------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------
 ...
 generate
   if (WB ==1 ) begin: gen_wb_support //support write buffer
     ...
     u_ahb_to_ahb_sync_down_wb (
         .HCLK            (HCLK),
         .HRESETn         (HRESETn),
         .HCLKEN          (1'b1), // Write buffer in fast AHB clock, therefore it is tied high
         ...
         .HRESPS          (HRESPS),
         .HRDATAS         (HRDATAS),
         .BWERR           (BWERR),  // Bufferable write error signal, valid for one HCLK cycle
   ...
   else begin: gen_no_wb_support   // write buffer is not includes
 ...
 // --------------------------------------------------------------------------------
 cmsdk_ahb_to_ahb_sync_down_core
 #(
  .AW(AW),
  .DW(DW),
  .MW(MW),
