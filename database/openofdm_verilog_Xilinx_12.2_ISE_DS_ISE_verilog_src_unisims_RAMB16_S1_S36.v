...
// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/unisims/RAMB16_S1_S36.v,v 1.10 2007/02/22 01:58:05 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module RAMB16_S1_S36 (DOA, DOB, DOPB, ADDRA, ADDRB, CLKA, CLKB, DIA, DIB, DIPB, ENA, ENB, SSRA, SSRB, WEA, WEB);
	...
	case (SIM_COLLISION_CHECK)
	    ...
	    "ALL" : ;
	    default : begin
		          $display("Attribute Syntax Error : The Attribute SIM_COLLISION_CHECK on RAMB16_S1_S36 instance %m is set to %s.  Legal values for this attribute are ALL, NONE, WARNING_ONLY or GENERATE_X_ONLY.", SIM_COLLISION_CHECK);
		          $finish;
	              end
...
//		6'b101010 : begin data_collision_b_a <= 2'b00; display_wa_rb; end
    ...
    always @(posedge memory_collision) begin
	if ((output_flag || display_flag)) begin
	    mem[addra_int[13:5]][addra_int[4:0] * 1 +: 1] <= 1'bx;
	    memory_collision <= 0;
	end
    ...
    always @(posedge memory_collision_a_b) begin
	if ((output_flag || display_flag)) begin
	    mem[addra_reg[13:5]][addra_reg[4:0] * 1 +: 1] <= 1'bx;
	    memory_collision_a_b <= 0;
	end
    end
	...
	if (ssrb_int == 0 && output_flag) begin
	    dob_out[addra_int[4:0] * 1 +: 1] <= #100 1'bX;
	end
	data_collision[0] <= 0;
    end
    ...
    always @(posedge data_collision_a_b[0]) begin
	if (ssrb_int == 0 && output_flag) begin
	    dob_out[addra_reg[4:0] * 1 +: 1] <= #100 1'bX;
	end
	data_collision_a_b[0] <= 0;
    end
	...
	if (ena_int == 1'b1) begin
		...
		if (wea_int == 1'b1) begin
			...
			doa_out <= #100 mem[addra_int[13:5]][addra_int[4:0] * 1 +: 1];
	    ...
	    // memory
	    if (wea_int == 1'b1) begin
		mem[addra_int[13:5]][addra_int[4:0] * 1 +: 1] <= dia_int;
	    end
