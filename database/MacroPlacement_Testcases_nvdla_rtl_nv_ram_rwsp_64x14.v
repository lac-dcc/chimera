...
// This wrapper consists of : 1 Ram cells: RAMDP_64X14_GL_M2_E2 ;
...
nv_ram_rwsp_64x14_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_64x14 (
...
integer row;
begin
// initializing RAMDP_64X14_GL_M2_E2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_64x14.ram_Inst_64X14.mem_write(row - 0, shadow_mem[row][13:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [13:0] data;
begin
// probing RAMDP_64X14_GL_M2_E2
 if (row >= 0 && row < 64) data[13:0] = r_nv_ram_rwsp_64x14.ram_Inst_64X14.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 896);
  row = random_num / 14;
