...
//
// Revision:
//    01/20/09 - Initial version for serial configuration simulation model.
...
module SIM_CONFIG_V6_SERIAL ( 
  ...
  initial begin
    if (DEVICE_ID == 32'h0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  SIM_CONFIG_V6_SERIAL instance %m is not set.");
      $finish;
     end
   end
    ...
    if (prog_b_t == 0)
       ...
       if (prog_b_t == 1)  begin
          ...
          if (m_in != 3'b111) begin
             mode_sample_flag <=  0;
             $display("Error: input M is %h. Only Slave Serial mode M=111 supported on SIM_CONFIG_V6_SERIAL instance %m.", m_in);
          end
          else
  ...
  always @(posedge init_b_t )
       if (prog_b_t != 1)  begin
            if ($time != 0 )
       $display("Error: PROGB is not high when INITB goes high on SIM_CONFIG_V6_SERIAL instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on SIM_CONFIG_V6_SERIAL instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in == 0) 
       ...
       if (prog_pulse_low < cfg_Tprog )
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on SIM_CONFIG_V6_SERIAL instance %m at time %t.", cfg_Tprog, $time);
    end
           ...
           if (conti_data_flag == 1 ) begin
               ...
               5'b01100 : begin
                          ...
                          if (pack_in_reg[27:0] != DEVICE_ID[27:0]) begin
                             id_error_flag <= 1;
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on SIM_CONFIG_V6_SERIAL instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                          end 
                          else
                             id_error_flag <= 0;
                          end
