...
//
// Revision:
//    03/02/09 - Initial version of serial configuration simulation model for
//               Spartan3A.
// End Revision
////////////////////////////////////////////////////////////////////////////////
...
module SIM_CONFIG_S3A_SERIAL (
  ...
  wire GSR, GTS, GWE;
  wire cclk_in, init_b_in, prog_b_in;
...
//  buf buf_dout (DOUT, ds_out);
  ...
  reg [15:0] tmp_dword1, tmp_dword2;
  integer wr_bit_addr;

  initial begin
    if (DEVICE_ID == 32'h0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  SIM_CONFIG_S3A_SERIAL instance %m is not set.");
//      $finish;
     end
   end
  ...
  always @(posedge init_b_t or negedge prog_b_t) 
    if (prog_b_t==0)
       ...
       if (prog_b_t)  begin
          ...
          if (m_in != 3'b111) begin
             $display("Error: input M is %h. Only Slave Serial mode M=111 supported on SIM_CONFIG_S3A_SERIAL instance %m.", m_in);
//            $finish;
          end
       end     
       else if ($time != 0)
       $display("Error: PROGB is not high when INITB goes high on SIM_CONFIG_S3A_SERIAL instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on SIM_CONFIG_S3A_SERIAL instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in ==0) 
       ...
       if (prog_pulse_low < cfg_Tprog )
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on SIM_CONFIG_S3A_SERIAL instance %m at time %t.", cfg_Tprog, $time);
    end

//    assign rw_en = (mode_sample_flag == 1 ) ? 1 : 0;
      ...
      if (desync_flag) begin
       ...
       if (rw_en == 1 ) begin
          if (bus_sync_flag == 0) begin
               tmp_dword1 = {tmp_dword2[14:0], ds_in};
               if (tmp_dword1[15:0] == 16'hAA99) begin
                          bus_sync_flag <= 1;
                          new_data_in_flag <= 0;
                          tmp_dword2 <= 16'b0;
                          pack_in_reg <= 16'b0;
               ...
               6'b001110 : 
                          if (conti_data_cnt==5'b00001) begin
                             ...
                             if (idcode_tmp[27:0] != DEVICE_ID[27:0]) begin
                                id_error_flag <= 1;
                                $display("Error :  written value to IDCODE register is %h which does not match DEVICE ID %h on SIM_CONFIG_S3A_SERIAL instance %m at time %t.", idcode_tmp, DEVICE_ID, $time);
                             end 
                             else
                             id_error_flag <= 0;
                          end
...
//                           csbo_cnt <= pack_in_reg[4:0];
             ...
             if (pack_in_reg[15:13] != 3'b001 && downcont == 0 ) begin
//                $display("Warning :  only Type 1 Packet supported on SIM_CONFIG_S3A_SERIAL instance %m at time %t.", $time);
                cmd_wr_flag <= 0;
                type2_flag <= 1;
                conti_data_flag <= 1;
