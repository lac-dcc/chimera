...
// This wrapper consists of : 1 Ram cells: RAMDP_20X16_GL_M1_E2 ;
...
nv_ram_rwsthp_20x16_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsthp_20x16 (
...
integer row;
begin
// initializing RAMDP_20X16_GL_M1_E2
for (row = 0; row < 20; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsthp_20x16.ram_Inst_20X16.mem_write(row - 0, shadow_mem[row][15:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [15:0] data;
begin
// probing RAMDP_20X16_GL_M1_E2
 if (row >= 0 && row < 20) data[15:0] = r_nv_ram_rwsthp_20x16.ram_Inst_20X16.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 320);
  row = random_num / 16;
