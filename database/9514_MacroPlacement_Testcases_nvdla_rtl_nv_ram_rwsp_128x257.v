...
// This wrapper consists of : 1 Ram cells: RAMPDP_128X258_GL_M1_D2 ;
...
wire [257:0] mbist_Do_r0_int_net;
...
nv_ram_rwsp_128x257_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_128x257 (
...
integer row;
begin
// initializing RAMPDP_128X258_GL_M1_D2
for (row = 0; row < 128; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_128x257.ram_Inst_128X258.mem_write(row - 0, shadow_mem[row][256:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [256:0] data;
begin
// probing RAMPDP_128X258_GL_M1_D2
 if (row >= 0 && row < 128) data[256:0] = r_nv_ram_rwsp_128x257.ram_Inst_128X258.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 32896);
  row = random_num / 257;
