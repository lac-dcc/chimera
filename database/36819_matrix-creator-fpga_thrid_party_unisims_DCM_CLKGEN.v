...
// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/stan/DCM_CLKGEN.v,v 1.20 2011/09/07 22:16:09 yanx Exp $
///////////////////////////////////////////////////////////////////////////////
...
module DCM_CLKGEN (
  ...
  reg progdone_out_tmp = 1;
  wire progdone_out;
  reg progdone_out_u = 1;
  reg lk_pd = 0;
  reg lk_pd1 = 0;
  reg lk_pd0 = 0;
  reg clkfx_clk = 0;
...
//
  ...
  always @(posedge lk_pd0 or posedge rst_lk or posedge dcm_en_lk 
        or posedge rst_ms) 
   if (rst_ms == 1) begin
       locked_out <= 0;
       locked_out_g <= 0;
       ...
       locked_out_g <= #(lk_delay) lk_pd0;
       lk_pd1 <= #1 lk_pd0; 
       lk_pd <= #2 lk_pd0; 
   end
 
  
   assign locked_out_ms = locked_out & ~clkin_ls_out;

   assign locked_out_ms1 =  ((spa >= 0  && spa <= 2) ||  (spa >= 3 && spa <= 5 && spse == 0)) ? (locked_out_g & ~clkin_ls_out) : 0;
...
//  always @(negedge clkfx_clk or rst_ms or rst_prog or lk_pd1)
  always @(negedge clkfx_clk or rst_ms  or lk_pd1 or lk_pd or posedge spse1 )
    ...
    else if (lk_pd1 == 1 && lk_pd == 0) begin
      spju = 0;
      spst = 0;
      pdh_fx = pd_fx;
      pdh_fx_t = pd_fx;
...
//
   ...
   assign progdone_out = progdone_out_tmp & !rst_in;
  ...
  always @(negedge progen_in or posedge rst_in) 
  if (rst_in == 1) begin
     pg_m_reg = CLKFX_MULTIPLY - 1;
     pg_d_reg = CLKFX_DIVIDE - 1;
  end 
  else
  ...
  always @(posedge go_cmd  or posedge rst_in) 
  if (rst_in == 1) begin
     rst_lk <= 0;
     spse0 <= 0;
