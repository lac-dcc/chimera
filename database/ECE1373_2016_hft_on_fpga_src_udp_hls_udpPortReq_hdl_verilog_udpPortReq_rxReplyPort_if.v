...
module udpPortReq_rxReplyPort_if (
    // AXI4-Stream singals
    input  wire       ACLK,
    input  wire       ARESETN,
    input  wire       TVALID,
    ...
    // User signals
    output wire [0:0] rxReplyPort_V_dout,
    output wire       rxReplyPort_V_empty_n,
    input  wire       rxReplyPort_V_read
);
//------------------------Local signal-------------------
// FIFO
wire [0:0] fifo_write;
wire [0:0] fifo_full_n;
wire [0:0] rxReplyPort_V_din;
wire [0:0] rxReplyPort_V_full_n;
// register slice
wire [0:0] s_valid;
wire [0:0] s_ready;
wire [0:0] s_data;
...
//------------------------Instantiation------------------
// rs
udpPortReq_rxReplyPort_reg_slice #(
    .N       ( 1 )
) rs (
    .clk     ( ACLK ),
    .reset   ( ARESETN ),
...
// rxReplyPort_V_fifo
udpPortReq_rxReplyPort_fifo #(
    .DATA_BITS  ( 1 ),
    .DEPTH_BITS ( 4 )
) rxReplyPort_V_fifo (
    .clk        ( ACLK ),
    .aclr       ( ~ARESETN ),
    .empty_n    ( rxReplyPort_V_empty_n ),
    .full_n     ( rxReplyPort_V_full_n ),
    .read       ( rxReplyPort_V_read ),
    .write      ( fifo_write ),
    .dout       ( rxReplyPort_V_dout ),
    .din        ( rxReplyPort_V_din )
);
...
assign rxReplyPort_V_din = m_data[0:0];
assign fifo_full_n       = rxReplyPort_V_full_n;
...
module udpPortReq_rxReplyPort_fifo
#(parameter
    DATA_BITS  = 8,
    DEPTH_BITS = 4
)(
...
module udpPortReq_rxReplyPort_reg_slice
#(parameter
    N = 8   // data width
) (
    // system signals
