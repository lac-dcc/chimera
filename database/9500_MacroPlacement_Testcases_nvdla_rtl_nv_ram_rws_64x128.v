...
// nv_ram_rws_64x128: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: RAMPDP_64X128_GL_M1_D2 ;
...
nv_ram_rws_64x128_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_64x128 (
...
integer row;
begin
// initializing RAMPDP_64X128_GL_M1_D2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_64x128.ram_Inst_64X128.mem_write(row - 0, shadow_mem[row][127:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [127:0] data;
begin
// probing RAMPDP_64X128_GL_M1_D2
 if (row >= 0 && row < 64) data[127:0] = r_nv_ram_rws_64x128.ram_Inst_64X128.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
