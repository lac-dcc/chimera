...
//lpm_counter DEVICE_FAMILY="Cyclone II" lpm_direction="UP" lpm_port_updown="PORT_UNUSED" lpm_width=6 aclr aload clock data q
...
`endif
	...
	wire  [5:0]   wire_counter_reg_bit1a_regout;
	wire  [5:0]   wire_counter_reg_bit1a_sdata;
	wire  [5:0]  a_data;
	wire  [5:0]  a_val;
	wire  aclr_actual;
	wire aset;
	wire clk_en;
	wire cnt_en;
	wire  [5:0]  effective_clrn;
	wire  [5:0]  effective_prn;
	wire  external_cin;
	wire  [5:0]  latch_signal;
	wire  [5:0]  pre_hazard;
	wire  [5:0]  pre_latch_signal;
	wire  [5:0]  s_val;
	wire  [5:0]  safe_q;
	wire sclr;
	wire sload;
	...
	assign
		wire_counter_reg_bit1a_sdata = ((~ latch_signal) ^ ({6{(~ sclr)}} & (({6{sset}} & s_val) | ({6{(~ sset)}} & data))));
	assign
		a_data = (({6{aset}} & a_val) | ({6{(~ aset)}} & data)),
		a_val = {6{1'b1}},
		aclr_actual = ((aclr | aset) | aload),
		aset = 1'b0,
		clk_en = 1'b1,
		cnt_en = 1'b1,
		effective_clrn = (~ ({6{aclr}} | ((~ a_data) & {6{(aload | aset)}}))),
		effective_prn = (~ ({6{(aload | aset)}} & a_data)),
		external_cin = 1'b1,
		...
		q = safe_q,
		s_val = {6{1'b1}},
		safe_q = ((((pre_hazard & latch_signal) & effective_clrn) | (((~ latch_signal) & (~ pre_hazard)) & effective_clrn)) | ({6{(~ aclr)}} & (~ effective_prn))),
		sclr = 1'b0,
		sload = 1'b0,
		sset = 1'b0,
		updown_dir = 1'b1;
...
module lpm_counter32 (
	...
	lpm_counter32_cntr	lpm_counter32_cntr_component (
				.aload (aload),
				.aclr (aclr),
				.clock (clock),
				.data (data),
