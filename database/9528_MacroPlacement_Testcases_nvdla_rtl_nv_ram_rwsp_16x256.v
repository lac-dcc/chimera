...
nv_ram_rwsp_16x256_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_16x256 (
...
for (row = 0; row < 16; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_16x256.ram_Inst_16X256.mem_write(row - 0, shadow_mem[row][255:0]);
shadow_written = 'b0;
end
endtask
`endif
...
// probing RAMDP_16X256_GL_M1_E2
 if (row >= 0 && row < 16) data[255:0] = r_nv_ram_rwsp_16x256.ram_Inst_16X256.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
