...
// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/rainier/RAMB36SDP.v,v 1.16 2007/06/15 20:58:40 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module RAMB36SDP (DBITERR, DO, DOP, ECCPARITY, SBITERR, 
    ...
    ARAMB36_INTERNAL INT_RAMB (.DIA(64'b0), .ENA(RDEN), .WEA(8'b0), .SSRA(SSR), .ADDRA({1'b0, RDADDR, 6'b0}), .CLKA(RDCLK), .DOA(DO), .DIB(DI), .ENB(WREN), .WEB(WE), .SSRB(1'b0), .ADDRB({1'b0, WRADDR, 6'b0}), .CLKB(WRCLK), .DOB(dangle_out32), .GSR(GSR), .DOPA(DOP), .DOPB(dangle_out4), .DIPA(4'b0), .DIPB(DIP), .CASCADEOUTLATA(dangle_out), .CASCADEOUTLATB(dangle_out), .CASCADEOUTREGA(dangle_out), .CASCADEOUTREGB(dangle_out), .CASCADEINLATA(1'b0), .CASCADEINLATB(1'b0), .CASCADEINREGA(1'b0), .CASCADEINREGB(1'b0), .REGCEA(REGCE), .REGCEB(1'b0), .REGCLKA(RDCLK), .REGCLKB(1'b0), .DBITERR(DBITERR), .ECCPARITY(ECCPARITY), .SBITERR(SBITERR));
