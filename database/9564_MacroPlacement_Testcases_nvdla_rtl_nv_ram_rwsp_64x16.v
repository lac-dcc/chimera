...
// This wrapper consists of : 1 Ram cells: RAMDP_64X16_GL_M2_E2 ;
...
nv_ram_rwsp_64x16_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_64x16 (
...
integer row;
begin
// initializing RAMDP_64X16_GL_M2_E2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_64x16.ram_Inst_64X16.mem_write(row - 0, shadow_mem[row][15:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [15:0] data;
begin
// probing RAMDP_64X16_GL_M2_E2
 if (row >= 0 && row < 64) data[15:0] = r_nv_ram_rwsp_64x16.ram_Inst_64X16.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
