...
// nv_ram_rws_32x32: synthesizable model wrapper
...
nv_ram_rws_32x32_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_32x32 (
...
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_32x32.ram_Inst_32X32.mem_write(row - 0, shadow_mem[row][31:0]);
shadow_written = 'b0;
end
endtask
`endif
...
// probing RAMDP_32X32_GL_M1_E2
 if (row >= 0 && row < 32) data[31:0] = r_nv_ram_rws_32x32.ram_Inst_32X32.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
