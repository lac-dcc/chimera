...
//cpsr_z
//不能在指令执行的中间对cpsr_z进行更改，原因在于条件执行可能基于cpsr_z，如果在指令执行中间对cpsr_z修改，那么会破坏条件执行。
//因此，使用multl_z对MULTL长乘指令的低32bit是否等于0进行保存。	
always @ ( posedge clk or posedge rst )
if ( rst )
    multl_z <= #`DEL 1'b0;
else if (cpu_en)
...
//同cpsr_n类似，区别在于multl指令，它需要判断两次(sum_rn_rm==32'b0)；
always @ ( * )
if ( cmd_ok & cmd_is_dp & cmd[20] )
...
//当然在乘法时，shift_word又有它用，可以参考乘法指令的实现
...
//hold_en表示多周期指令执行，swp会有一个周期的hold_en，mult/multl有有限的多周期，ldm会根据sum_m的递减结果而定
...
//乘法操作相关
/******************************************************/
//shift_word里面存放的是Rm，乘法操作使用shift_word
assign reg_rm =  shift_word;

//另外使用reg_rs存放Rs。如果是带符号的长乘，需要对Rm和Rs进行取反加一，得到绝对值
always @ ( posedge clk or posedge rst )
if ( rst )
    reg_rs <= #`DEL 32'd0;
else if ( cpu_en )
...
//mult_ans是reg_rm和reg_rs相乘的结果：
//mult指令会让sum_m==1，也就是说mult指令会在两个周期内完成，第一个周期完成乘法(sum_m==1)，也就是mult_ans计算得出。
//         其中mult_ans[31:0]会放在reg_rs寄存器内。reg_rn会根据mult指令是否加上rn，让reg_rn等于0或真正的Rn(rt给出)
//         而加法器的另外一个操作数sec_operand会强制link到reg_rs上，那么在(sum_m==0)时，完成一次32位的加法，即可完成mult指令
//multl指令会让sum_m==2，也就是multl指令会在三个周期内完成：
//          sum_m==2，也就是第一个周期时，完成32*32的乘法操作，64bit结果放在{reg_rm,reg_rs}内
//          sum_m==1，也就是第二个周期时，会把reg_rn([15:12])对应的Rn和第二操作数(reg_rs)相加减（根据乘数的符号而定），加法的结果写入Rnlo，进位保存入multl_extra_num，
//                     为了下一个周期完成高32bit加法，此时会做下面操作：reg_rn会把index从[15:12]换成[19:16]，reg_rm放的是高32bit位，此时写入reg_rs内。
//          sum_m==0，也就是第三个周期时，reg_rn会和reg_rs利用加法器相加，此时的进位采用multl_extra_num，结果放入Rnhi。
