...
module	slowfil_srl #(
		...
		// }}}
		// New sample input(s)--a new sample comes in any time i_ce
		// {{{
		// is true.  There must be at least NTAPS idle's between every
	...
	generate for (i = 1; i < MEMSZ; i=i+1)
	begin : GEN_DSRL
		always @(posedge i_clk)
		if (i_ce)
			dsrl[i] <= dsrl[i-1];
