...
// This wrapper consists of : 1 Ram cells: RAMDP_32X64_GL_M1_E2 ;
...
nv_ram_rws_32x64_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_32x64 (
...
integer row;
begin
// initializing RAMDP_32X64_GL_M1_E2
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_32x64.ram_Inst_32X64.mem_write(row - 0, shadow_mem[row][63:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [63:0] data;
begin
// probing RAMDP_32X64_GL_M1_E2
 if (row >= 0 && row < 32) data[63:0] = r_nv_ram_rws_32x64.ram_Inst_32X64.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
