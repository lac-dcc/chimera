...
//-----------------------------------------------------------
// Housekeeping interface for Caravan
//-----------------------------------------------------------
...
//
// Updated and revised, 5/19/2023:
// Extended the clock monitor routing from GPIO 14 and 15 to
// also route to GPIO 30 and 31.  This is done because the
// Caravan chip has no GPIO at positions 14 and 15.  For this
// revision, housekeeping.v has been copied to housekeeping_alt.v
// and modified for the extension, so that the Caravel design
// does not need to be modified.  For future versions, this
// version of housekeeping should be the only one.  Existing
// functionality for Caravel is not affected (the implementation
...
`endif
	    ...
	    8'h1b : fdata = {3'b000,
				clk1_output_dest[1], clk2_output_dest[1],
				clk1_output_dest[0], clk2_output_dest[0],
				trap_output_dest};
    ...
    assign mgmt_gpio_out[29:16] = mgmt_gpio_data[29:16];
...
`endif
    ...
    assign mgmt_gpio_out_15_prebuff = (clk2_output_dest[0] == 1'b1) ? user_clock
		: mgmt_gpio_data[15];
...
`endif
    ...
    assign mgmt_gpio_out_14_prebuff = (clk1_output_dest[0] == 1'b1) ? wb_clk_i
		: mgmt_gpio_data[14];
...
`endif
    ...
    assign mgmt_gpio_out_31_prebuff = (clk2_output_dest[1] == 1'b1) ? user_clock
		: mgmt_gpio_data[31];

(* keep *) sky130_fd_sc_hd__clkbuf_8 mgmt_gpio_31_buff_inst (
`ifdef USE_POWER_PINS
        .VPWR(VPWR),
        .VGND(VGND),
        .VPB(VPWR),
        .VNB(VGND),
`endif
	.A(mgmt_gpio_out_31_prebuff),
    .X(mgmt_gpio_out[31]));

    assign mgmt_gpio_out_30_prebuff = (clk1_output_dest[1] == 1'b1) ? wb_clk_i
		: mgmt_gpio_data[30];

(* keep *) sky130_fd_sc_hd__clkbuf_8 mgmt_gpio_30_buff_inst (
`ifdef USE_POWER_PINS
        .VPWR(VPWR),
        .VGND(VGND),
        .VPB(VPWR),
...
`define GPIO_LOAD	2'b11
	...
	if (porb == 1'b0) begin
	    ...
	    clk1_output_dest <= 2'b00;
	    clk2_output_dest <= 2'b00;
...
`endif
		    ...
		    /* Register 1a (power monitor) is read-only */
            	    ...
            	    8'h1b: begin
			clk1_output_dest[1] <= cdata[4];
			clk2_output_dest[1] <= cdata[3];
			clk1_output_dest[0] <= cdata[2];
			clk2_output_dest[0] <= cdata[1];
