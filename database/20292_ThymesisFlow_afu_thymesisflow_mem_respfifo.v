...
// @@@  Module Declaration
// ==============================================================================================================================
module mem_respfifo #(parameter integer WIDTH = 158) (
  ...
  , input              network_ready     // When 1, This module can return credits and push data. 
  ...
  // Output from FIFO  
  , input              cmd_tready        // When 1, the other side is ready to accept data
...
always @(posedge(clock))
  begin
    ...
    else if (cmd_tready == 1'b1 && rdptr != 3'b111 && cmdfifo_val[rdptr] == 1'b1 && (network_ready == 1'b1)) //push flit cond
      rdptr <= rdptr + 3'b001;                              // Move to next location, this one is complete
    else if (cmd_tready == 1'b1 && rdptr == 3'b111 && cmdfifo_val[rdptr] == 1'b1 && (network_ready == 1'b1))
...
always @(posedge(clock))
  begin 
    if (reset == 1'b1)
      cmdfifo_val <= 8'h00;                                              
    else if (cmd_in_valid == 1'b0 && cmd_tready == 1'b1 && cmdfifo_val[rdptr] == 1'b1 && (network_ready == 1'b1))
      cmdfifo_val <= cmdfifo_val & ~(8'h01 << rdptr);                       // When done, set rdptr bit to 0 
    else if (cmd_in_valid == 1'b1 && cmd_tready == 1'b1 && cmdfifo_val[rdptr] == 1'b1 && (network_ready == 1'b1))
...
assign cmd_out_valid = ((cmdfifo_val[rdptr] == 1'b1) && (network_ready == 1'b1) /*&& (cmd_tready == 1'b1)*/)? 1'b1:1'b0;
assign cmd_out       = ((cmdfifo_val[rdptr] == 1'b1) && (network_ready == 1'b1) /*&& (cmd_tready == 1'b1)*/) ? cmdfifo[rdptr] : {WIDTH{1'b0}};

// Check for overflow
assign fifo_overflow   = 1'b0; //(cmd_in_valid == 1'b1 && cmdfifo_val == 8'hFF) ? 1'b1 : 1'b0;
...
always @(posedge(clock))
  begin
    if (reset == 1'b1 || resync_credits == 1'b1)     
      initial_credits <= 4'b1000;                   // Initialize to number of FIFO entries 7 credits so wptr cannot overlap with rdptr in push mode.
    ...
    else if (cmd_tready == 1'b1 && cmdfifo_val[rdptr] == 1'b1 && initial_credits > 4'b0000)
...
always @(posedge(clock))
  if (reset == 1'b1 || resync_credits == 1'b1)   // Stop sending credits on either reset or resync_credits
    ...
    cmd_credit_to_TLX_q <= (tlx_is_ready == 1'b1 && ((
                            (cmd_tready == 1'b1) && (cmdfifo_val[rdptr] == 1'b1) && (network_ready == 1'b1)) || (initial_credits > 4'b0000 && ok_to_send_credits == 1'b1))
                           ) ? 1'b1 : 1'b0;
