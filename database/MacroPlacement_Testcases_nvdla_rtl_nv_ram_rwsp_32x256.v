...
nv_ram_rwsp_32x256_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_32x256 (
...
for (row = 0; row < 32; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_32x256.ram_Inst_32X256.mem_write(row - 0, shadow_mem[row][255:0]);
shadow_written = 'b0;
end
endtask
`endif
...
// probing RAMPDP_32X256_GL_M1_D2
 if (row >= 0 && row < 32) data[255:0] = r_nv_ram_rwsp_32x256.ram_Inst_32X256.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
