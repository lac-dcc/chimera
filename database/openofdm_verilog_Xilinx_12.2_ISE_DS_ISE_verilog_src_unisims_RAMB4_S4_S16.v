...
// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/unisims/RAMB4_S4_S16.v,v 1.9 2007/02/22 01:58:06 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module RAMB4_S4_S16 (DOA, DOB, ADDRA, ADDRB, CLKA, CLKB, DIA, DIB, ENA, ENB, RSTA, RSTB, WEA, WEB);
	...
	case (SIM_COLLISION_CHECK)
	    ...
	    "ALL" : ;
	    default : begin
		          $display("Attribute Syntax Error : The Attribute SIM_COLLISION_CHECK on RAMB4_S4_S16 instance %m is set to %s.  Legal values for this attribute are ALL, NONE, WARNING_ONLY or GENERATE_X_ONLY.", SIM_COLLISION_CHECK);
		          $finish;
	              end
...
module RAMB4_S4_S16 (DOA, DOB, ADDRA, ADDRB, CLKA, CLKB, DIA, DIB, ENA, ENB, RSTA, RSTB, WEA, WEB);
	...
	if ((output_flag || display_flag)) begin
	    for (mi = 0; mi < 16; mi = mi + 4) begin
		if ((mem_addra_int) == (mem_addrb_int + mi)) begin
		    mem[addra_int[9:2]][addra_int[1:0] * 4 +: 4] <= 4'bx;
		end
	    end
	    memory_collision <= 0;
	end
	...
	if ((output_flag || display_flag)) begin
	    for (mi = 0; mi < 16; mi = mi + 4) begin
		if ((mem_addra_reg) == (mem_addrb_int + mi)) begin
		    mem[addra_reg[9:2]][addra_reg[1:0] * 4 +: 4] <= 4'bx;
		end
	    end
	    memory_collision_a_b <= 0;
	end
	...
	if (ena_int == 1'b1) begin
		...
		if (wea_int == 1'b1)
		    ...
		    doa_out <= #100 mem[addra_int[9:2]][addra_int[1:0] * 4 +: 4];
	    end

	    // memory
	    if (wea_int == 1'b1) begin
		mem[addra_int[9:2]][addra_int[1:0] * 4 +: 4] <= dia_int;
	    end
