...
// nv_ram_rwsp_8x129: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: RAMDP_8X130_GL_M1_E2 ;
...
nv_ram_rwsp_8x129_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_8x129 (
...
integer row;
begin
// initializing RAMDP_8X130_GL_M1_E2
for (row = 0; row < 8; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_8x129.ram_Inst_8X130.mem_write(row - 0, shadow_mem[row][128:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [128:0] data;
begin
// probing RAMDP_8X130_GL_M1_E2
 if (row >= 0 && row < 8) data[128:0] = r_nv_ram_rwsp_8x129.ram_Inst_8X130.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 1032);
  row = random_num / 129;
