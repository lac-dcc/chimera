...
//        end
    ...
    always @(*) begin
        if ( ( ((Rs1_E == RD_W) & RegWriteW) & (Rs1_E != 0) ) /*| lwStall == 1'b1 */) begin
                ForwardAE = 2'b01; //forward from memory to alu, need data from memory into alu, recently written data in mem required
        end
        else if ( (((Rs1_E == RD_M) & RegWriteM) & (Rs1_E != 0) ) /*& lwStall == 1'b1*/ ) begin
            ForwardAE = 2'b10;//forward from alu output to alu input, recently calculated data in alu required
        end
        else begin
            ForwardAE = 2'b00;
        end        
