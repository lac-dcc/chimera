...
// This wrapper consists of : 1 Ram cells: RAMDP_8X258_GL_M1_E2 ;
...
nv_ram_rwsp_8x257_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rwsp_8x257 (
...
integer row;
begin
// initializing RAMDP_8X258_GL_M1_E2
for (row = 0; row < 8; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rwsp_8x257.ram_Inst_8X258.mem_write(row - 0, shadow_mem[row][256:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [256:0] data;
begin
// probing RAMDP_8X258_GL_M1_E2
 if (row >= 0 && row < 8) data[256:0] = r_nv_ram_rwsp_8x257.ram_Inst_8X258.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
...
integer bitnum;
begin
  random_num = rflip.rollpli(0, 2056);
  row = random_num / 257;
