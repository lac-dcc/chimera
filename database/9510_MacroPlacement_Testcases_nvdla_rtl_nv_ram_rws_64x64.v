...
nv_ram_rws_64x64_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_64x64 (
...
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_64x64.ram_Inst_64X64.mem_write(row - 0, shadow_mem[row][63:0]);
shadow_written = 'b0;
end
endtask
`endif
...
// probing RAMPDP_64X64_GL_M1_D2
 if (row >= 0 && row < 64) data[63:0] = r_nv_ram_rws_64x64.ram_Inst_64X64.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
