...
// nv_ram_rws_64x256: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: RAMPDP_64X256_GL_M1_D2 ;
...
nv_ram_rws_64x256_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_64x256 (
...
integer row;
begin
// initializing RAMPDP_64X256_GL_M1_D2
for (row = 0; row < 64; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_64x256.ram_Inst_64X256.mem_write(row - 0, shadow_mem[row][255:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [255:0] data;
begin
// probing RAMPDP_64X256_GL_M1_D2
 if (row >= 0 && row < 64) data[255:0] = r_nv_ram_rws_64x256.ram_Inst_64X256.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
