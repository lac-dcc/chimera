...
module mem_cmd_fifo #(parameter integer WIDTH = 550) (
  ...
  , output [3:0]       data_buffers_available  // Number of buffer slots available (1 buffer holds response and data FLIT (if used))
  , output             data_fifo_full          // : when 0 Fifo is full -  active low
    // Output from FIFO
...
reg [WIDTH-1:0] datafifo [7:0];   // 16 row array
...
reg      [7:0] datafifo_val;      // 1=associated entry is used, 0=associated entry is open and can be overwritten

wire fifo_full;
assign fifo_full = (datafifo_val == 8'hFF) ? 1'b1 : 1'b0;

// Manage write pointer
always @(posedge(clock))
  begin
    if (reset == 1'b1) 
      wrptr <= 3'b000;                                      // Don't allow it to become same with read
    else if (data_in_valid == 1'b1 && wrptr != 3'b111 && ((fifo_full == 1'b0) || ((fifo_full == 1'b1) && (data_sent == 1'b1))))  //as long as not full   
      wrptr <= wrptr + 3'b001;                              // Add an entry, so increment
    else if (data_in_valid == 1'b1 && wrptr == 3'b111  && ((fifo_full == 1'b0) || ((fifo_full == 1'b1) && (data_sent == 1'b1))))   
...
always @(posedge(clock))
  begin
    ...
    else if (data_sent == 1'b1 && rdptr != 3'b111)
...
always @(posedge(clock))
  begin
    if (reset == 1'b1)  
      datafifo_val <= 8'h00;                                                     // Initialize to all 0, all rows are free
    else if (data_in_valid == 1'b1 && data_sent == 1'b0 && (fifo_full == 1'b0)) //if full is 1 (i.e. table is full, do not receive.  
      datafifo_val <= datafifo_val | (8'h01 << wrptr);                           // When loading, set wrptr bit to 1
    else if (data_in_valid == 1'b0 && data_sent == 1'b1)
      datafifo_val <= datafifo_val & ~(8'h01 << rdptr);                          // When done, set rdptr bit to 0 
      ...
      datafifo_val <= (datafifo_val | (8'h01 << wrptr)) & ~(8'h01 << rdptr);  // Both set and clear a bit 
    else
...
//As long as it is full don't accept anything. improve to implemente a  moving full window.  
...
// Provide data directly out of array (if timing permits) to align with changing of rdptr with dataect to resp_sent
...
reg [3:0] data_buffers_used;
always @(*)   // Combinational
  if (datafifo_val == 8'h0000)               // Empty FIFO
    data_buffers_used = 4'b0000; 
    ...
    data_buffers_used = (4'b1000 + {1'b0,wrptr}) - {1'b0,rdptr};   
  else // wrptr == rdptr                      // Pointers are equal, but FIFO is not empty so it must be full
    data_buffers_used = 4'b1000; 
assign data_buffers_available = 4'b1000 - data_buffers_used;
