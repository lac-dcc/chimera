...
// nv_ram_rws_16x128: synthesizable model wrapper
...
// This wrapper consists of : 1 Ram cells: RAMDP_16X128_GL_M1_E2 ;
...
nv_ram_rws_16x128_logic #(FORCE_CONTENTION_ASSERTION_RESET_ACTIVE) r_nv_ram_rws_16x128 (
...
integer row;
begin
// initializing RAMDP_16X128_GL_M1_E2
for (row = 0; row < 16; row = row + 1)
 if (shadow_written[row]) r_nv_ram_rws_16x128.ram_Inst_16X128.mem_write(row - 0, shadow_mem[row][127:0]);
shadow_written = 'b0;
end
endtask
`endif
...
reg [127:0] data;
begin
// probing RAMDP_16X128_GL_M1_E2
 if (row >= 0 && row < 16) data[127:0] = r_nv_ram_rws_16x128.ram_Inst_16X128.mem_read(row - 0);
    probe_mem_val = data;
end
endfunction
`endif
