...
void LivenessVisitor::visit(Terminal *node) {
  if (isStartingToken(node->getElement())) {
    ...
    while (identifiersInScope.size() > scopeLimit.top()) {
...
void LivenessVisitor::visit(Genericidentifier *node) {
  ...
  && node->getChildren()[0]->getElement().find("module") == std::string::npos){
   ...
   void
  LivenessVisitor::visit(Param_type_followed_by_id_and_dimensions_opt *node){
    defaultVisitor(node);
  };
   ...
   void LivenessVisitor::visit(List_of_ports_or_port_declarations_trailing_comma_non_ansi
                         *node){
                            defaultVisitor(node);
                         };
   ...
   void
  LivenessVisitor::visit(Data_type_or_implicit_basic_followed_by_id_and_dimensions_opt *node)
    {
        defaultVisitor(node);
    };
   ...
   void
  LivenessVisitor::visit(List_of_ports_or_port_declarations_item_last_ansi *node){
    defaultVisitor(node);
  };

   void
  LivenessVisitor::visit(Data_type_or_implicit_followed_by_id_and_dimensions_opt *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(Non_anonymous_gate_instance_or_register_variable_list *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(List_of_ports_or_port_declarations_non_ansi *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(Non_anonymous_gate_instance_or_register_variable *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(List_of_ports_or_port_declarations_trailing_comma_ansi *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(List_of_ports_or_port_declarations_item_last_non_ansi *node){
    defaultVisitor(node);
  };
   ...
   void
  LivenessVisitor::visit(Type_identifier_or_implicit_followed_by_id_and_dimensions_opt *node)
    {
        defaultVisitor(node);
    };
   ...
   void
  LivenessVisitor::visit(Module_parameter_port_list_preprocessor_last *node){
    defaultVisitor(node);
  };
