#include "TypeInference.h"

constraintSet TypeInferenceVisitor::visit(Node *node){}

constraintSet TypeInferenceVisitor::visit(Terminal *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_delay_range_expr *node){}

constraintSet TypeInferenceVisitor::visit(Port *node){}

constraintSet TypeInferenceVisitor::visit(Specify_simple_path *node){}

constraintSet TypeInferenceVisitor::visit(Event_control *node){}

constraintSet TypeInferenceVisitor::visit(String_literal *node){}

constraintSet TypeInferenceVisitor::visit(Charge_strength_opt *node){}

constraintSet TypeInferenceVisitor::visit(Dpi_import_export *node){}

constraintSet TypeInferenceVisitor::visit(For_initialization_opt *node){}

constraintSet TypeInferenceVisitor::visit(List_of_port_identifiers *node){}

constraintSet TypeInferenceVisitor::visit(Non_integer_type *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_ranges_opt *node){}

constraintSet TypeInferenceVisitor::visit(Package_item_no_pp *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_delay_repetition_list *node){}

constraintSet TypeInferenceVisitor::visit(Add_expr *node){}

constraintSet TypeInferenceVisitor::visit(Dpi_import_item *node){}

constraintSet TypeInferenceVisitor::visit(Tk_realtime *node){}

constraintSet TypeInferenceVisitor::visit(Case_item *node){}

constraintSet TypeInferenceVisitor::visit(Property_if_else_expr *node){}

constraintSet TypeInferenceVisitor::visit(Non_anonymous_instantiation_base *node){}

constraintSet TypeInferenceVisitor::visit(Unary_expr *node){}

constraintSet TypeInferenceVisitor::visit(Unary_op *node){}

constraintSet TypeInferenceVisitor::visit(Class_id *node){}

constraintSet TypeInferenceVisitor::visit(Reference *node){}

constraintSet TypeInferenceVisitor::visit(Cast *node){}

constraintSet TypeInferenceVisitor::visit(Tf_variable_identifier_first *node){}

constraintSet TypeInferenceVisitor::visit(Type_identifier_or_implicit_basic_followed_by_id_and_dimensions_opt
        *node){};

constraintSet TypeInferenceVisitor::visit(Type_or_id_root *node){}

constraintSet TypeInferenceVisitor::visit(Module_or_generate_item *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port_list_trailing_comma *node){}

constraintSet TypeInferenceVisitor::visit(Instantiation_type *node){}

constraintSet TypeInferenceVisitor::visit(Parameters *node){}

constraintSet TypeInferenceVisitor::visit(Reference_or_call_base *node){}

constraintSet TypeInferenceVisitor::visit(Param_type_followed_by_id_and_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Structure_or_array_pattern_key *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_assign_list *node){}

constraintSet TypeInferenceVisitor::visit(Defparam_assign_list *node){}

constraintSet TypeInferenceVisitor::visit(Generate_item_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tk_octdigits *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_expr_primary *node){}

constraintSet TypeInferenceVisitor::visit(Lpvalue *node){}

constraintSet TypeInferenceVisitor::visit(Label_opt *node){}

constraintSet TypeInferenceVisitor::visit(Escapedidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Generate_item *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_primitive *node){}

constraintSet TypeInferenceVisitor::visit(Spec_reference_event *node){}

constraintSet TypeInferenceVisitor::visit(Udp_input_declaration_list *node){}

constraintSet TypeInferenceVisitor::visit(Non_port_module_item *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_trailing_comma_non_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Property_prefix_expr *node){}

constraintSet TypeInferenceVisitor::visit(Expr_primary_braces *node){}

constraintSet TypeInferenceVisitor::visit(Pow_expr *node){}

constraintSet TypeInferenceVisitor::visit(For_init_decl_or_assign *node){}

constraintSet TypeInferenceVisitor::visit(Udp_initial *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_list_paren_opt *node){}

constraintSet TypeInferenceVisitor::visit(Struct_data_type *node){}

constraintSet TypeInferenceVisitor::visit(Udp_sequ_entry *node){}

constraintSet TypeInferenceVisitor::visit(Net_variable_or_decl_assign *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_byname_list_trailing_comma *node){}

constraintSet TypeInferenceVisitor::visit(Inc_or_dec_expression *node){}

constraintSet TypeInferenceVisitor::visit(Net_type *node){}

constraintSet TypeInferenceVisitor::visit(Select_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Spec_polarity *node){}

constraintSet TypeInferenceVisitor::visit(Macro_formal_parameter *node){}

constraintSet TypeInferenceVisitor::visit(Structure_or_array_pattern_expression *node){}

constraintSet TypeInferenceVisitor::visit(For_step *node){}

constraintSet TypeInferenceVisitor::visit(Specparam_list *node){}

constraintSet TypeInferenceVisitor::visit(Tk_binbase *node){}

constraintSet TypeInferenceVisitor::visit(Tk_stringliteral *node){}

constraintSet TypeInferenceVisitor::visit(Var_opt *node){}

constraintSet TypeInferenceVisitor::visit(Identifier_optional_unpacked_dimensions *node){}

constraintSet TypeInferenceVisitor::visit(Function_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Type_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Udp_body *node){}

constraintSet TypeInferenceVisitor::visit(Mul_expr *node){}

constraintSet TypeInferenceVisitor::visit(Par_block *node){}

constraintSet TypeInferenceVisitor::visit(Keywordidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Logeq_expr *node){}

constraintSet TypeInferenceVisitor::visit(Module_common_item *node){}

constraintSet TypeInferenceVisitor::visit(Primitive_gate_instance *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_or_implicit *node){}

constraintSet TypeInferenceVisitor::visit(Tk_octbase *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_expr *node){}

constraintSet TypeInferenceVisitor::visit(Tk_hexbase *node){}

constraintSet TypeInferenceVisitor::visit(Property_expr *node){}

constraintSet TypeInferenceVisitor::visit(Seq_block *node){}

constraintSet TypeInferenceVisitor::visit(Task_item *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_or_implicit_basic_followed_by_id_and_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specify_block *node){}

constraintSet TypeInferenceVisitor::visit(Tk_decnumber *node){}

constraintSet TypeInferenceVisitor::visit(Tk_decbase *node){}

constraintSet TypeInferenceVisitor::visit(Property_implication_expr *node){}

constraintSet TypeInferenceVisitor::visit(Bit_logic_opt *node){}

constraintSet TypeInferenceVisitor::visit(Integer_atom_type *node){}

constraintSet TypeInferenceVisitor::visit(Lifetime *node){}

constraintSet TypeInferenceVisitor::visit(Any_argument_list *node){}

constraintSet TypeInferenceVisitor::visit(Delay_value_list *node){}

constraintSet TypeInferenceVisitor::visit(Module_port_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Jump_statement *node){}

constraintSet TypeInferenceVisitor::visit(Delay_value *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_or_expr *node){}

constraintSet TypeInferenceVisitor::visit(Module_attribute_foreign_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_direction_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_item_expr_opt *node){}

constraintSet TypeInferenceVisitor::visit(Cont_assign_list *node){}

constraintSet TypeInferenceVisitor::visit(Any_argument_list_trailing_comma *node){}

constraintSet TypeInferenceVisitor::visit(Procedural_continuous_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Expression_or_dist *node){}

constraintSet TypeInferenceVisitor::visit(Module_start *node){}

constraintSet TypeInferenceVisitor::visit(Expr_primary_parens *node){}

constraintSet TypeInferenceVisitor::visit(Open_range_list *node){}

constraintSet TypeInferenceVisitor::visit(Unary_prefix_expr *node){}

constraintSet TypeInferenceVisitor::visit(Delay_identifier *node){}

constraintSet TypeInferenceVisitor::visit(List_of_identifiers_unpacked_dimensions *node){}

constraintSet TypeInferenceVisitor::visit(Delay_scope *node){}

constraintSet TypeInferenceVisitor::visit(Action_block *node){}

constraintSet TypeInferenceVisitor::visit(Trailing_assign_opt *node){}

constraintSet TypeInferenceVisitor::visit(Class_item *node){}

constraintSet TypeInferenceVisitor::visit(Enum_name_list_trailing_comma *node){}

constraintSet TypeInferenceVisitor::visit(Udp_port_list *node){}

constraintSet TypeInferenceVisitor::visit(Equiv_impl_expr *node){}

constraintSet TypeInferenceVisitor::visit(Class_new *node){}

constraintSet TypeInferenceVisitor::visit(Port_net_type *node){}

constraintSet TypeInferenceVisitor::visit(Unqualified_id *node){}

constraintSet TypeInferenceVisitor::visit(Begin *node){}

constraintSet TypeInferenceVisitor::visit(System_tf_call *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_byname_list_item_last *node){}

constraintSet TypeInferenceVisitor::visit(Conditional_generate_construct *node){}

constraintSet TypeInferenceVisitor::visit(Timescale_directive *node){}

constraintSet TypeInferenceVisitor::visit(Port_expression *node){}

constraintSet TypeInferenceVisitor::visit(Udp_sequ_entry_list *node){}

constraintSet TypeInferenceVisitor::visit(Symbol_or_label *node){}

constraintSet TypeInferenceVisitor::visit(Simple_sequence_expr *node){}

constraintSet TypeInferenceVisitor::visit(Systemtfidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Tk_unbasednumber *node){}

constraintSet TypeInferenceVisitor::visit(Decl_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Class_items_opt *node){}

constraintSet TypeInferenceVisitor::visit(Enum_name *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specify_edge_path *node){}

constraintSet TypeInferenceVisitor::visit(Implicit_class_handle *node){}

constraintSet TypeInferenceVisitor::visit(Module_item *node){}

constraintSet TypeInferenceVisitor::visit(Statement_item *node){}

constraintSet TypeInferenceVisitor::visit(Type_identifier_followed_by_id *node){}

constraintSet TypeInferenceVisitor::visit(Method_prototype *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_item *node){}

constraintSet TypeInferenceVisitor::visit(Block_item_or_statement_or_null_list *node){}

constraintSet TypeInferenceVisitor::visit(Endnew_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specify_item_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specify_simple_path_decl *node){}

constraintSet TypeInferenceVisitor::visit(Tk_rs_eq *node){}

constraintSet TypeInferenceVisitor::visit(Package_import_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Udp_port_decl *node){}

constraintSet TypeInferenceVisitor::visit(With_exprs_suffix *node){}

constraintSet TypeInferenceVisitor::visit(Generate_region *node){}

constraintSet TypeInferenceVisitor::visit(Call_base *node){}

constraintSet TypeInferenceVisitor::visit(Array_locator_method *node){}

constraintSet TypeInferenceVisitor::visit(Port_declaration_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Bitand_expr *node){}

constraintSet TypeInferenceVisitor::visit(Range_list_in_braces *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port_list_item_last *node){}

constraintSet TypeInferenceVisitor::visit(Hex_based_number *node){}

constraintSet TypeInferenceVisitor::visit(Hierarchy_event_identifier *node){}

constraintSet TypeInferenceVisitor::visit(List_of_variable_decl_assignments *node){}

constraintSet TypeInferenceVisitor::visit(Random_qualifier_opt *node){}

constraintSet TypeInferenceVisitor::visit(Drive_strength_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tk_timeliteral *node){}

constraintSet TypeInferenceVisitor::visit(Value_range *node){}

constraintSet TypeInferenceVisitor::visit(Expr_mintypmax_generalized *node){}

constraintSet TypeInferenceVisitor::visit(Polarity_operator *node){}

constraintSet TypeInferenceVisitor::visit(Package_item *node){}

constraintSet TypeInferenceVisitor::visit(For_initialization *node){}

constraintSet TypeInferenceVisitor::visit(Enum_data_type *node){}

constraintSet TypeInferenceVisitor::visit(Blocking_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Delay1 *node){}

constraintSet TypeInferenceVisitor::visit(Based_number *node){}

constraintSet TypeInferenceVisitor::visit(Procedural_timing_control_statement *node){}

constraintSet TypeInferenceVisitor::visit(Block_item_decl *node){}

constraintSet TypeInferenceVisitor::visit(Net_variable_or_decl_assigns *node){}

constraintSet TypeInferenceVisitor::visit(Time_literal *node){}

constraintSet TypeInferenceVisitor::visit(Function_item_data_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Decl_variable_dimension *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Endfunction_label_opt *node){}

constraintSet TypeInferenceVisitor::visit(Qualified_id *node){}

constraintSet TypeInferenceVisitor::visit(Immediate_assertion_statement *node){}

constraintSet TypeInferenceVisitor::visit(Join_keyword *node){}

constraintSet TypeInferenceVisitor::visit(Tk_bindigits *node){}

constraintSet TypeInferenceVisitor::visit(Event_trigger *node){}

constraintSet TypeInferenceVisitor::visit(Dynamic_array_new *node){}

constraintSet TypeInferenceVisitor::visit(Matches_expr *node){}

constraintSet TypeInferenceVisitor::visit(Port_reference_list *node){}

constraintSet TypeInferenceVisitor::visit(Array_reduction_method *node){}

constraintSet TypeInferenceVisitor::visit(Builtin_array_method *node){}

constraintSet TypeInferenceVisitor::visit(Package_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Assignment_pattern_expression *node){}

constraintSet TypeInferenceVisitor::visit(Drive_strength *node){}

constraintSet TypeInferenceVisitor::visit(Port_declaration_non_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port_list *node){}

constraintSet TypeInferenceVisitor::visit(Var_type *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_opt *node){}

constraintSet TypeInferenceVisitor::visit(Module_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Genericidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Package_item_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_item_last_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_or_implicit_followed_by_id_and_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Xor_expr *node){}

constraintSet TypeInferenceVisitor::visit(Dec_based_number *node){}

constraintSet TypeInferenceVisitor::visit(Loop_statement *node){}

constraintSet TypeInferenceVisitor::visit(Data_declaration_or_module_instantiation *node){}

constraintSet TypeInferenceVisitor::visit(Udp_comb_entry_list *node){}

constraintSet TypeInferenceVisitor::visit(Bit_logic *node){}

constraintSet TypeInferenceVisitor::visit(Reference_or_call *node){}

constraintSet TypeInferenceVisitor::visit(Local_root *node){}

constraintSet TypeInferenceVisitor::visit(Port_direction *node){}

constraintSet TypeInferenceVisitor::visit(Variable_decl_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Specify_edge_path_decl *node){}

constraintSet TypeInferenceVisitor::visit(Function_prototype *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_within_expr *node){}

constraintSet TypeInferenceVisitor::visit(Expr_primary_no_groups *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_byname *node){}

constraintSet TypeInferenceVisitor::visit(Case_any *node){}

constraintSet TypeInferenceVisitor::visit(Spec_notifier *node){}

constraintSet TypeInferenceVisitor::visit(Block_item_or_statement_or_null *node){}

constraintSet TypeInferenceVisitor::visit(Specify_item *node){}

constraintSet TypeInferenceVisitor::visit(Any_param_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_list_trailing_comma *node){}

constraintSet TypeInferenceVisitor::visit(Struct_union_member *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_positional *node){}

constraintSet TypeInferenceVisitor::visit(Dir *node){}

constraintSet TypeInferenceVisitor::visit(Lifetime_opt *node){}

constraintSet TypeInferenceVisitor::visit(List_of_identifiers *node){}

constraintSet TypeInferenceVisitor::visit(Edge_operator *node){}

constraintSet TypeInferenceVisitor::visit(Tk_virtual_opt *node){}

constraintSet TypeInferenceVisitor::visit(Timeunits_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Tf_item_or_statement_or_null *node){}

constraintSet TypeInferenceVisitor::visit(Package_import_list *node){}

constraintSet TypeInferenceVisitor::visit(Expr_primary *node){}

constraintSet TypeInferenceVisitor::visit(Signed_unsigned_opt *node){}

constraintSet TypeInferenceVisitor::visit(Non_anonymous_gate_instance_or_register_variable_list *node){}

constraintSet TypeInferenceVisitor::visit(Udp_comb_entry *node){}

constraintSet TypeInferenceVisitor::visit(Delay3 *node){}

constraintSet TypeInferenceVisitor::visit(Inc_or_dec_or_primary_expr *node){}

constraintSet TypeInferenceVisitor::visit(Procedural_assertion_statement *node){}

constraintSet TypeInferenceVisitor::visit(Struct_union_member_list *node){}

constraintSet TypeInferenceVisitor::visit(Comp_expr *node){}

constraintSet TypeInferenceVisitor::visit(Postfix_expression *node){}

constraintSet TypeInferenceVisitor::visit(Symbolidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Udp_input_sym *node){}

constraintSet TypeInferenceVisitor::visit(Pos_neg_number *node){}

constraintSet TypeInferenceVisitor::visit(Specify_terminal_descriptor *node){}

constraintSet TypeInferenceVisitor::visit(Select_variable_dimension *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Generate_case_items *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_primitive_scalar *node){}

constraintSet TypeInferenceVisitor::visit(Tf_item_or_statement_or_null_list *node){}

constraintSet TypeInferenceVisitor::visit(Conditional_statement *node){}

constraintSet TypeInferenceVisitor::visit(Specparam_decl *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_non_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Statement_or_null_list *node){}

constraintSet TypeInferenceVisitor::visit(Task_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Data_declaration_base *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Macronumericwidth *node){}

constraintSet TypeInferenceVisitor::visit(Trailing_decl_assignment_opt *node){}

constraintSet TypeInferenceVisitor::visit(Gate_instance_or_register_variable *node){}

constraintSet TypeInferenceVisitor::visit(Bind_instantiation *node){}

constraintSet TypeInferenceVisitor::visit(Package_import_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Module_package_import_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Structure_or_array_pattern_expression_list *node){}

constraintSet TypeInferenceVisitor::visit(Event_expression_list *node){}

constraintSet TypeInferenceVisitor::visit(Tk_rss_eq *node){}

constraintSet TypeInferenceVisitor::visit(Implements_interface_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Delay3_opt *node){}

constraintSet TypeInferenceVisitor::visit(Expr_mintypmax_trans_set *node){}

constraintSet TypeInferenceVisitor::visit(Statement_or_null *node){}

constraintSet TypeInferenceVisitor::visit(Enum_name_list *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_unary_expr *node){}

constraintSet TypeInferenceVisitor::visit(Pp_identifier *node){}

constraintSet TypeInferenceVisitor::visit(Shift_expr *node){}

constraintSet TypeInferenceVisitor::visit(Oct_based_number *node){}

constraintSet TypeInferenceVisitor::visit(Hierarchy_segment *node){}

constraintSet TypeInferenceVisitor::visit(Nonblocking_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Expression_list_proper *node){}

constraintSet TypeInferenceVisitor::visit(Generate_if *node){}

constraintSet TypeInferenceVisitor::visit(Function_item *node){}

constraintSet TypeInferenceVisitor::visit(Scope_prefix *node){}

constraintSet TypeInferenceVisitor::visit(Primitive_gate_instance_list *node){}

constraintSet TypeInferenceVisitor::visit(Simple_immediate_assertion_statement *node){}

constraintSet TypeInferenceVisitor::visit(Description_list *node){}

constraintSet TypeInferenceVisitor::visit(Dr_strength0 *node){}

constraintSet TypeInferenceVisitor::visit(Unique_priority_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specparam *node){}

constraintSet TypeInferenceVisitor::visit(Port_expression_opt *node){}

constraintSet TypeInferenceVisitor::visit(Cond_expr *node){}

constraintSet TypeInferenceVisitor::visit(Tk_reg_opt *node){}

constraintSet TypeInferenceVisitor::visit(Logor_expr *node){}

constraintSet TypeInferenceVisitor::visit(Delay3_or_drive_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tf_item_or_statement_or_null_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Net_decl_assign *node){}

constraintSet TypeInferenceVisitor::visit(Always_any *node){}

constraintSet TypeInferenceVisitor::visit(List_of_tf_variable_identifiers *node){}

constraintSet TypeInferenceVisitor::visit(Var_or_net_type_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_list_item_last *node){}

constraintSet TypeInferenceVisitor::visit(Task_declaration_id *node){}

constraintSet TypeInferenceVisitor::visit(Instantiation_base *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_repetition_expr *node){}

constraintSet TypeInferenceVisitor::visit(Tk_hexdigits *node){}

constraintSet TypeInferenceVisitor::visit(Non_anonymous_gate_instance_or_register_variable *node){}

constraintSet TypeInferenceVisitor::visit(Localparam_assign *node){}

constraintSet TypeInferenceVisitor::visit(Class_items *node){}

constraintSet TypeInferenceVisitor::visit(Expression_in_parens *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_trailing_comma_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Port_reference *node){}

constraintSet TypeInferenceVisitor::visit(Dist_opt *node){}

constraintSet TypeInferenceVisitor::visit(Caseeq_expr *node){}

constraintSet TypeInferenceVisitor::visit(Generate_block *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_item_last_non_ansi *node){}

constraintSet TypeInferenceVisitor::visit(Any_argument_list_item_last *node){}

constraintSet TypeInferenceVisitor::visit(Gate_instantiation *node){}

constraintSet TypeInferenceVisitor::visit(Casting_type *node){}

constraintSet TypeInferenceVisitor::visit(Expr_mintypmax *node){}

constraintSet TypeInferenceVisitor::visit(Continuous_assign *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_assign *node){}

constraintSet TypeInferenceVisitor::visit(Delay_value_simple *node){}

constraintSet TypeInferenceVisitor::visit(Switchtype *node){}

constraintSet TypeInferenceVisitor::visit(Package_or_generate_item_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Bin_based_number *node){}

constraintSet TypeInferenceVisitor::visit(Loop_generate_construct *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_override *node){}

constraintSet TypeInferenceVisitor::visit(Udp_primitive *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_expr *node){}

constraintSet TypeInferenceVisitor::visit(Repeat_control *node){}

constraintSet TypeInferenceVisitor::visit(Packed_signing_opt *node){}

constraintSet TypeInferenceVisitor::visit(Expression_or_null_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Enum_name_list_item_last *node){}

constraintSet TypeInferenceVisitor::visit(Macro_formals_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Spec_notifier_opt *node){}

constraintSet TypeInferenceVisitor::visit(Hierarchy_extension *node){}

constraintSet TypeInferenceVisitor::visit(Generate_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Data_declaration_modifiers_opt *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_throughout_expr *node){}

constraintSet TypeInferenceVisitor::visit(Property_expr_or_assignment_list *node){}

constraintSet TypeInferenceVisitor::visit(Bitor_expr *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_list *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_trailing_comma_named *node){}

constraintSet TypeInferenceVisitor::visit(Function_return_type_and_id *node){}

constraintSet TypeInferenceVisitor::visit(Statement *node){}

constraintSet TypeInferenceVisitor::visit(Dr_strength1 *node){}

constraintSet TypeInferenceVisitor::visit(Assign_modify_statement *node){}

constraintSet TypeInferenceVisitor::visit(Udp_port_decls *node){}

constraintSet TypeInferenceVisitor::visit(Integer_vector_type *node){}

constraintSet TypeInferenceVisitor::visit(Assignment_pattern *node){}

constraintSet TypeInferenceVisitor::visit(Class_constructor *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port *node){}

constraintSet TypeInferenceVisitor::visit(Module_or_generate_item_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Final_construct *node){}

constraintSet TypeInferenceVisitor::visit(Member_name *node){}

constraintSet TypeInferenceVisitor::visit(Class_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Package_import_item *node){}

constraintSet TypeInferenceVisitor::visit(Signing *node){}

constraintSet TypeInferenceVisitor::visit(Class_constructor_prototype *node){}

constraintSet TypeInferenceVisitor::visit(Generate_case_item *node){}

constraintSet TypeInferenceVisitor::visit(Data_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Expression *node){}

constraintSet TypeInferenceVisitor::visit(Cont_assign *node){}

constraintSet TypeInferenceVisitor::visit(Delay_or_event_control_opt *node){}

constraintSet TypeInferenceVisitor::visit(Bind_directive *node){}

constraintSet TypeInferenceVisitor::visit(Misc_directive *node){}

constraintSet TypeInferenceVisitor::visit(Udp_initial_expr_opt *node){}

constraintSet TypeInferenceVisitor::visit(Description *node){}

constraintSet TypeInferenceVisitor::visit(Data_type_base *node){}

constraintSet TypeInferenceVisitor::visit(Trailing_assign *node){}

constraintSet TypeInferenceVisitor::visit(Module_end *node){}

constraintSet TypeInferenceVisitor::visit(Wait_statement *node){}

constraintSet TypeInferenceVisitor::visit(Package_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Case_items *node){}

constraintSet TypeInferenceVisitor::visit(Statement_or_null_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Port_named *node){}

constraintSet TypeInferenceVisitor::visit(Udp_output_sym *node){}

constraintSet TypeInferenceVisitor::visit(Assignment_statement_no_expr *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_item_last_positional *node){}

constraintSet TypeInferenceVisitor::visit(Module_item_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_and_expr *node){}

constraintSet TypeInferenceVisitor::visit(Data_type *node){}

constraintSet TypeInferenceVisitor::visit(Argument_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(For_step_opt *node){}

constraintSet TypeInferenceVisitor::visit(Sequence_intersect_expr *node){}

constraintSet TypeInferenceVisitor::visit(Function_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Net_type_or_none *node){}

constraintSet TypeInferenceVisitor::visit(Class_declaration_extends_opt *node){}

constraintSet TypeInferenceVisitor::visit(Boolean_abbrev_opt *node){}

constraintSet TypeInferenceVisitor::visit(Specify_path_identifiers *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_direction *node){}

constraintSet TypeInferenceVisitor::visit(Tk_decdigits *node){}

constraintSet TypeInferenceVisitor::visit(End *node){}

constraintSet TypeInferenceVisitor::visit(Net_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Tk_tagged_opt *node){}

constraintSet TypeInferenceVisitor::visit(Tk_xzdigits *node){}

constraintSet TypeInferenceVisitor::visit(Gatetype *node){}

constraintSet TypeInferenceVisitor::visit(Preprocess_include_argument *node){}

constraintSet TypeInferenceVisitor::visit(Macro_formals_list *node){}

constraintSet TypeInferenceVisitor::visit(Const_opt *node){}

constraintSet TypeInferenceVisitor::visit(Logand_expr *node){}

constraintSet TypeInferenceVisitor::visit(Event_expression *node){}

constraintSet TypeInferenceVisitor::visit(Udp_entry_list *node){}

constraintSet TypeInferenceVisitor::visit(Preprocessor_action *node){}

constraintSet TypeInferenceVisitor::visit(Tf_port_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Udp_init_opt *node){}

constraintSet TypeInferenceVisitor::visit(Type_identifier_or_implicit_followed_by_id_and_dimensions_opt *node){}

constraintSet TypeInferenceVisitor::visit(Source_text *node){}

constraintSet TypeInferenceVisitor::visit(Defparam_assign *node){}

constraintSet TypeInferenceVisitor::visit(Decl_dimensions *node){}

constraintSet TypeInferenceVisitor::visit(Constant_dec_number *node){}

constraintSet TypeInferenceVisitor::visit(Case_statement *node){}

constraintSet TypeInferenceVisitor::visit(Assignment_statement *node){}

constraintSet TypeInferenceVisitor::visit(Dpi_import_property_opt *node){}

constraintSet TypeInferenceVisitor::visit(Dpi_spec_string *node){}

constraintSet TypeInferenceVisitor::visit(Trailing_decl_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Always_construct *node){}

constraintSet TypeInferenceVisitor::visit(Genvar_opt *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_named *node){}

constraintSet TypeInferenceVisitor::visit(Subroutine_call *node){}

constraintSet TypeInferenceVisitor::visit(Property_expr_or_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Any_argument *node){}

constraintSet TypeInferenceVisitor::visit(Module_or_interface_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Expression_opt *node){}

constraintSet TypeInferenceVisitor::visit(Gate_instance_or_register_variable_list *node){}

constraintSet TypeInferenceVisitor::visit(Tk_evalstringliteral *node){}

constraintSet TypeInferenceVisitor::visit(Delay_or_event_control *node){}

constraintSet TypeInferenceVisitor::visit(Udp_input_list *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_value_byname_list *node){}

constraintSet TypeInferenceVisitor::visit(Module_port_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(List_of_ports_or_port_declarations_opt *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_trailing_comma_positional *node){}

constraintSet TypeInferenceVisitor::visit(Block_item_or_statement_or_null_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Genvar_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Number *node){}

constraintSet TypeInferenceVisitor::visit(Specify_item_list *node){}

constraintSet TypeInferenceVisitor::visit(Tk_ls_eq *node){}

constraintSet TypeInferenceVisitor::visit(Tf_variable_identifier *node){}

constraintSet TypeInferenceVisitor::visit(Initial_construct *node){}

constraintSet TypeInferenceVisitor::visit(Localparam_assign_list *node){}

constraintSet TypeInferenceVisitor::visit(Disable_statement *node){}

constraintSet TypeInferenceVisitor::visit(List_of_module_item_identifiers *node){}

constraintSet TypeInferenceVisitor::visit(Any_port_list_item_last_named *node){}

constraintSet TypeInferenceVisitor::visit(Net_variable *node){}

constraintSet TypeInferenceVisitor::visit(Parameter_expr_list *node){}

constraintSet TypeInferenceVisitor::visit(Specparam_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Deferred_immediate_assertion_statement *node){}

constraintSet TypeInferenceVisitor::visit(Slice_size_opt *node){}

constraintSet TypeInferenceVisitor::visit(Concurrent_assertion_item *node){}

constraintSet TypeInferenceVisitor::visit(Clocking_item_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Cycle_delay *node){}

constraintSet TypeInferenceVisitor::visit(Macroiditem *node){}

constraintSet TypeInferenceVisitor::visit(Assume_property_statement *node){}

constraintSet TypeInferenceVisitor::visit(Pull01 *node){}

constraintSet TypeInferenceVisitor::visit(Preprocessor_directive *node){}

constraintSet TypeInferenceVisitor::visit(Module_item_directive *node){}

constraintSet TypeInferenceVisitor::visit(Assert_property_statement *node){}

constraintSet TypeInferenceVisitor::visit(Scope_or_if_res *node){}

constraintSet TypeInferenceVisitor::visit(Task_prototype *node){}

constraintSet TypeInferenceVisitor::visit(Clocking_declaration *node){}

constraintSet TypeInferenceVisitor::visit(Block_identifier_opt *node){}

constraintSet TypeInferenceVisitor::visit(Macroidentifier *node){}

constraintSet TypeInferenceVisitor::visit(Property_spec_disable_iff_opt *node){}

constraintSet TypeInferenceVisitor::visit(Macrogenericitem *node){}

constraintSet TypeInferenceVisitor::visit(Module_block *node){}

constraintSet TypeInferenceVisitor::visit(Stream_expression *node){}

constraintSet TypeInferenceVisitor::visit(Final_or_zero *node){}

constraintSet TypeInferenceVisitor::visit(Tk_edge_descriptor *node){}

constraintSet TypeInferenceVisitor::visit(Method_property_qualifier_list_not_starting_with_virtual*node){}

constraintSet TypeInferenceVisitor::visit(Identifier_opt *node){}

constraintSet TypeInferenceVisitor::visit(Module_parameter_port_list_preprocessor_last *node){}

constraintSet TypeInferenceVisitor::visit(Type_assignment *node){}

constraintSet TypeInferenceVisitor::visit(Type_assignment_list *node){}

constraintSet TypeInferenceVisitor::visit(Method_qualifier_list_opt *node){}

constraintSet TypeInferenceVisitor::visit(Stream_operator *node){}

constraintSet TypeInferenceVisitor::visit(Assertion_item *node){}

constraintSet TypeInferenceVisitor::visit(Property_spec *node){}

constraintSet TypeInferenceVisitor::visit(Streaming_concatenation *node){}

constraintSet TypeInferenceVisitor::visit(Edge_descriptor_list *node){}

constraintSet TypeInferenceVisitor::visit(Property_qualifier *node){}

constraintSet TypeInferenceVisitor::visit(Event_control_opt *node){}

constraintSet TypeInferenceVisitor::visit(Class_item_qualifier *node){}

constraintSet TypeInferenceVisitor::visit(Concurrent_assertion_statement *node){}

constraintSet TypeInferenceVisitor::visit(Cover_property_statement *node){}

constraintSet TypeInferenceVisitor::visit(Stream_expression_list *node){}

constraintSet TypeInferenceVisitor::visit(Class_item_qualifier_list_opt *node){}