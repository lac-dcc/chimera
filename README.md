...
### 4. Train probabilistic grammar {#train-probabilistic-grammar}
...
### How to generate multiple designs?
...
python3 change_probabilities.py --input grammar.json \
                           ...
                           --power 0.5
```
- `--input`: Path to the trained or pre-trained grammar file (from Step 4).
- `--output`: Path where the modified grammar will be saved.
- `--power`: Compression power (default 0.5).
  - <1.0 → flattens distribution (boosts rare productions).
